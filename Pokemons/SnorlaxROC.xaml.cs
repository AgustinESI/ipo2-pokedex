    ...
    public sealed partial class SnorlaxROC : UserControl, iPokemon
    {
        ...
        private async void a_uñas()
        {
            if (est_muerto == false)
            {
                ...
                mpSonidos1.Source = MediaSource.CreateFromUri(new Uri("ms-appx:///Assets/pokemons/SnorlaxROC/uñas.mp3"));
                mpSonidos1.Play();
        ...
        private async void a_terremoto()
        {
            if (est_muerto == false)
            {
                ...
                mpSonidos2.Source = MediaSource.CreateFromUri(new Uri("ms-appx:///Assets/pokemons/SnorlaxROC/derrumbe.mp3"));
        ...
        private void a_escudo()
        {
            if (est_muerto == false)
            {
                ...
                mpSonidos3.Source = MediaSource.CreateFromUri(new Uri("ms-appx:///Assets/pokemons/SnorlaxROC/escudo.mp3"));
                mpSonidos3.Play();
        ...
        private async void a_cansado()
        {
            if (est_muerto == false)
            {
                if (est_cansado == false)
                {
                    ...
                    mpSonidos4.Source = MediaSource.CreateFromUri(new Uri("ms-appx:///Assets/pokemons/SnorlaxROC/aDormir.mp3"));
                    mpSonidos4.Play();
        ...
        private async void a_bostedo_siesta_energia()
        {
            if (est_muerto == false)
            {
                if (est_herido == true || est_cansado == true)
                {
                    ...
                    mpSonidos5.Source = MediaSource.CreateFromUri(new Uri("ms-appx:///Assets/pokemons/SnorlaxROC/roncar.mp3"));
                    mpSonidos5.Play();
                    if (est_herido == true)
                    {
                        a_oriHer();
        ...
        private async void a_herido()
        {
            if (est_muerto == false)
            {
                if (est_herido == false)
                {
                    ...
                    mpSonidos6.Source = MediaSource.CreateFromUri(new Uri("ms-appx:///Assets/pokemons/SnorlaxROC/vidaBaja.mp3"));
                    mpSonidos6.Play();
        ...
        private async void a_derrotado()
        {
            if (est_muerto == false)
            {
                ...
                mpSonidos7.Source = MediaSource.CreateFromUri(new Uri("ms-appx:///Assets/pokemons/SnorlaxROC/muerte.mp3"));
                mpSonidos7.Play();
